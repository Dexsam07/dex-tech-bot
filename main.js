...
const { addCommandReaction, handleAutoreact } = require('./lib/reactions');
...
const { handleAntiforeign, antiforeignCommand } = require('./commands/antiforeign');
...
const { autorecordtypeCommand } = require('./commands/autorecordtype');
const { execute: antibotCommand } = require('./commands/antibot');
...
function getDeploymentPlatform() {
    ...
    else if (process.env.CODESPACE_NAME) return 'Codespaces';
    else if (process.env.PANEL_APP) return 'Panel';
    ...
    try {
        ...
        if (!isGroup && message.key.remoteJid?.endsWith('@lid')) {
            try {
                ...
                const realJid = message.key.participant || message.participant || message.verifiedBizAcc || null;
        ...
        // quoted message check (only for commands that need it)
        const quoteRequired = ['simage', 'sticker', 's', 'delete', 'del', 'take', 'steal', 'tourl', 'url', 'emojimix', 'emix', 'crop', 'vv', 'viewonce', 'setpp', 'setgpp', 'removebg', 'rmbg', 'nobg', 'remini', 'enhance', 'upscale', 'blur', 'takeout'];
        if (isCommand) {
            const cmdName = commandWithoutPrefix.split(/\s+/)[0].toLowerCase();
            if (quoteRequired.includes(cmdName)) {
                const quotedMsg = message.message?.extendedTextMessage?.contextInfo?.quotedMessage;
                if (!quotedMsg) {
                    await sock.sendMessage(chatId, { text: `❌ Please reply to a message with *${cmdName}* command.` });
                    return;
                }
                try {
                    const quotedId = message.message.extendedTextMessage.contextInfo.stanzaId;
                    const quotedMsgFull = await sock.loadMessage(message.key.remoteJid, quotedId);
                    if (!quotedMsgFull) {
                        await sock.sendMessage(chatId, { text: '❌ The quoted message has expired or was deleted. Please quote a newer message.' });
                        return;
                    }
                } catch (e) {
                    console.error('❌ Error checking quoted message:', e);
                }
            }
        }
        ...
        // We have a command
        const rawText = rawMessageText; // for slicing in cases
        const userMessage = commandWithoutPrefix.toLowerCase().trim();
                    ...
                    if (settings.timezone) {
                        try {
                            ...
                            if (moment.tz.zone(settings.timezone)) {
                                lastCheckTime = moment(updateStatus.lastCheck).tz(settings.timezone).format('YYYY-MM-DD HH:mm:ss');
                            } else {
                                lastCheckTime = updateStatus.lastCheck.toISOString() + ` (Invalid timezone: ${settings.timezone})`;
                            }
                        } catch (error) {
                            lastCheckTime = updateStatus.lastCheck.toLocaleString('en-US', { timeZone: settings.timezone }) || updateStatus.lastCheck.toISOString();
                        }
                    } else {
...
process.on('SIGINT', async () => {
    ...
    try { stopAllInfiniteRecordings(); } catch (e) {}
    try { stopAllInfiniteTyping(); } catch (e) {}
