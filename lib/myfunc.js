...
//════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════//
//* 
//  * file : lib/myfunc.js (Utility Functions)
//  * description : Common utility functions for bot
...
// ✅ Improved getBuffer with error handling
...
// ✅ Alias for getBuffer (same as getImg)
exports.getImg = exports.getBuffer;

// ✅ Improved fetchJson with better error
...
const DEFAULT_TIMEZONE = settings.timezone || 'Asia/Kolkata';

exports.getTime = (format, date) => {
    if (date) {
        return moment(date).locale('id').tz(DEFAULT_TIMEZONE).format(format);
    } else {
        return moment.tz(DEFAULT_TIMEZONE).locale('id').format(format);
    }
};
...
// ✅ Improved tanggal with dynamic timezone
exports.tanggal = (numer) => {
    ...
    const time = moment.tz(DEFAULT_TIMEZONE).format('DD/MM HH:mm:ss');
    const d = new Date();
...
// ✅ Fixed jam function (timezone param was missing)
exports.jam = (numer, options = {}) => {
    let format = options.format || "HH:mm";
    let timeZone = options.timeZone || DEFAULT_TIMEZONE;
...
exports.generateProfilePicture = async (buffer) => {
    try {
        ...
        console.error('❌ generateProfilePicture error:', err.message);
        throw err;
    }
};
...
exports.getSizeMedia = (path) => {
    return new Promise((resolve, reject) => {
        if (/^https?:\/\//.test(path)) {
            ...
            reject('Invalid input: must be URL or Buffer');
        }
    });
};
